...
class RecursiveChunker:
    ...
    def _split(self, current_text: str, remaining_separators: list[str]) -> list[str]:
        ...
        # Keep separators so joining the chunks reconstructs the original text.
        parts = re.split(r"(?<=" + re.escape(separator) + r")", current_text)
        chunks: list[str] = []
        pending = ""
        for part in parts:
            if not part:
                continue
            pieces = (self._split(part, remaining_separators[1:])
                      if len(part) > self.chunk_size else [part])
            for piece in pieces:
